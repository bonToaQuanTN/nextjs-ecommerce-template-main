...
import { productApi } from "@/service/map/inventory/inventory";
...
const ShopContent = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        let res;
        if (searchQuery.trim()) {
          res = await productApi.search(searchQuery, currentPage);
        } else {
  ...
  return (
    <>
      <Breadcrumb
        title={searchQuery ? `Kết quả tìm kiếm: "${searchQuery}"` : "Tất cả sản phẩm"}
        pages={["shop", "/", searchQuery ? "search" : "shop without sidebar"]}
      />
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            <div className="w-full">
              ...
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>
                      Đang hiển thị <span className="text-dark font-medium">{products.length}</span> trên tổng số kết quả
                    </p>
                  </div>
              ...
              >
                {loading ? (
                  // Bạn có thể thay bằng Skeleton Loading nếu muốn đẹp hơn
                  ...
                  <div className="col-span-full text-center py-20">
                    <p className="text-gray-500 text-lg">Không tìm thấy sản phẩm nào phù hợp với từ khóa "{searchQuery}"</p>
                  </div>
                ) : (
                    ...
                    <ul className="flex items-center gap-1">
                      <li>
                        <button
                          ...
                          type="button"
                          className="flex items-center justify-center w-9 h-9 ease-out duration-200 rounded-[3px] hover:text-white hover:bg-blue disabled:text-gray-4 disabled:hover:bg-white"
                        >
                      ...
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                        <li key={page}>
                          <button
                            ...
                            type="button"
                            className={`flex items-center justify-center w-9 h-9 ease-out duration-200 rounded-[3px] font-medium text-sm ${
                              currentPage === page
                                ? "bg-blue text-white"
  ...
  return (
    <Suspense fallback={
      <div className="flex justify-center items-center h-screen">
        <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-blue border-r-transparent align-[-0.125em]"></div>
      </div>
    }>
      <ShopContent />
    </Suspense>
